...
Task* createCOWTask(Task* parent)
{
    ...
    __asm__ __volatile__ ("movq %%rax,%0; movq %%rbx,%1; movq %%rcx,%3; movq %%rdx,%4; movq %%rsi,%5; movq %%rdi, %6; movq %%rbp,%7; pushfq; pop %8": "=g" (task->regs.rax),"=g" (task->regs.rbx),"=g" (task->regs.rcx),"=g" (task->regs.rdx),"=g" (task->regs.rsi),"=g" (task->regs.rdi),"=g" (task->regs.rax),"=g" (task->regs.rflags)::);
